...
def test_rdp_probe():
    ...
    events = [
        CanonicalLogEvent(event_id=f"e{i}", timestamp=datetime.now(), src_ip="1.2.3.4", dst_ip=f"10.0.0.{i}", dst_port=3389, action="block", parser_name="test", parse_status="success", protocol="TCP", tcp_flags="SYN")
        for i in range(2)
    ]
...
def test_ssh_probe():
    ...
    ctx = DetectionContext(settings=settings, analysis_started_at=datetime.now())
    events = [
        CanonicalLogEvent(event_id=f"e{i}", timestamp=datetime.now(), src_ip="1.2.3.4", dst_ip=f"10.0.0.{i}", dst_port=22, action="block", parser_name="test", parse_status="success", protocol="TCP", tcp_flags="SYN")
        for i in range(2)
    ]
    signals = rule.evaluate(events, ctx)
    assert len(signals) == 1
